...
public class NTupleSelector {
	...
	public final void init(String selection, String[] variables) {
		ExpressionBuilder builder = new ExpressionBuilder(selection).operator(operatorAND).operator(operatorOR).operator(operatorGT)
		        .operator(operatorLT).operator(operatorEQ);
